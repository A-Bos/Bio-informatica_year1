...
def main():
    sequence = getSequentie("startOpgave3.txt")
...
def one():
    DdeI_cut = sequence.find("ctgag")
    if DdeI_cut >= 0:
        print ("\n","DdeI does cut.")
        print ("Place: ",sequence.index("ctgag"))
    else:
        print ("\n","DdeI does not cut.")

def two():
    BspMII_cut = sequence.find("tccgga")
    if BspMII_cut >= 0:
        print ("\n","BspMII does cut.")
        print ("Place: ",sequence.index("tccgga"))
    else:
        print ("\n","BspMII does not cut.")

def three():
    EcoRI_cut = sequence.find("gaattc")
    if EcoRI_cut >= 0:
        print ("\n","EcoRI does cut.")
        print ("Place: ",sequence.index("gaattc"))
    else:
        print ("\n","EcoRI does not cut.")

def four():
    HindIII_cut = sequence.find("aagctt")
    if HindIII_cut >= 0:
        print ("\n","HindIII does cut.")
        print ("Place: ",sequence.index("aagctt"))
    else:
        print ("\n","HindIII does not cut.")

def five():
    TaqI_cut = sequence.find("tcga")
    if TaqI_cut >= 0:
        print ("\n","TaqI does cut.")
        print ("Place: ",sequence.index("tcga"))
    else:
        print ("\n","TaqI does not cut.")
main()
one()
